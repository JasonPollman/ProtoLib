    ...
    var libs = {
            ...
            camelize: function camelize (string) {
                ...
                libs.object.every(arguments, function (s) {
                    if(s) {
                        ...
                        s = s.toString().replace(/[^a-z0-9$]/gi, '_').split(/[\s_]+/g);
        ...
        array: {
            ...
            union: function union (a) {
                var args = libs.object.only(libs.object.makeArray(arguments), 'array');
            ...
            difference: function difference () {
                var arrays = libs.object.only(libs.object.makeArray(arguments), 'array');
                ...
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                // var intermediates = [];
                // for(i = 0; i < Math.ceil(arrays.length / 2); i++) {
                //     for(var n = Math.ceil(arrays.length / 2); n >= 0; n--) {
                //         if(arrays[i] !== arrays[n]) {
                //             var x = difference(arrays[i], arrays[n]);
                //             intermediates = difference(intermediates, x);
                //             console.log(intermediates, "<<<<<<<<<<<<");
                //         }
                //     }
                // }
                //
                // var intersection = libs.array.intersect.apply(null, arguments);
                // libs.object.each(intersection, function (val) {
                //     var idx = intermediates.indexOf(val);
        ...
        object: {
            ...
            each: function each (o, rangeA, rangeB, f) {
                ...
                var ret    = null,
                    ...
                    exit = function () {
                        broken   = true;
                        ret      = arguments.length > 1 ? libs.object.makeArray(arguments) : arguments[0];
                    };
            ...
            getCallback: function getCallback (argumentsObject) {
                var callback = function () {};
                for(var k = argumentsObject.length - 1; k >= 0; k--) {
                    if(argumentsObject[k] instanceof Function) {
                        callback = argumentsObject[k];
                        break;
                    }
                }
                return callback;
