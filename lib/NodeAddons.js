...
module.exports = function (protolib) {
    /**
     * The user home directory
     * @type {String}
     */
    protolib.USER_HOME = process.env[(process.platform === 'win32') ? 'USERPROFILE' : 'HOME'];

    /**
     * The path to the user's temp directory
     * @type {String}
     */
    protolib.TMPDIR = os.tmpdir();

    /**
     * The process arguments, all pretty like from minimist.
     * @type {Object}
     */
    protolib.ARGS = require('minimist')(process.argv.slice(2), { boolean: ['s', 'd', 'debug'] });

    /**
     * Whether or not we got the debug flag
     * @type {Boolean}
     */
    protolib.DEBUG = !!protolib.ARGS.debug;

    /**
     * The process environment
     * @type {String}
     */
    protolib.ENV = typeof protolib.ARGS.env === 'string' ? protolib.ARGS.env : typeof process.env.NODE_ENV === 'string' ? process.env.NODE_ENV : 'dev';
     ...
     * @function
     */
    protolib.string.hash = function hash (algorithm) {
        protolib.performWithCurrent(function (s) {
     ...
     * @function
     */
    protolib.string.md5 = function md5 () {
        protolib.performWithCurrent(function (s) {
            return s.hash('md5');
        });
    };
     ...
     * @function
     */
    protolib.string.wordWrapForTTY = function wordWrapForTTY (padleft, padright, omitFirst) {
        protolib.performWithCurrent(function (s) {
            return s[protolib.PROTO_IDENTIFIER].wordWrapToLength(process.stdout.columns, padleft, padright, omitFirst);
        });
    };
     ...
     * @function
     */
    protolib.object.hash = function hash (algorithm) {
        protolib.performWithCurrent(function (o) {
     ...
     * @function
     */
    protolib.object.md5 = function md5 () {
        protolib.performWithCurrent(function (o) {
     ...
     * @function
     */
    protolib.object.bytes = function bytes () {
        protolib.performWithCurrent(function (o) {
     ...
     * @todo Windows
     */
    protolib.getProcessPathByPID = function getProcessPathByPID (pid, done) {
        done = arguments[protolib.PROTO_IDENTIFIER].last() instanceof Function ? arguments[protolib.PROTO_IDENTIFIER].last() : protolib.NULLF;
        var e;
                    ...
                    default:
                        exec('ps -o comm= ' + pid, function (err, stdout, stderr) {
                            if(err || stderr) {
                                ...
                                reject(e);
                                done.call(protolib, e);
                            }
                            else if(stdout.trim() && stdout.trim() !== '') {
                                resolve(stdout);
                                done.call(protolib, stdout);
                            }
                            else {
     ...
     */
    protolib.parseJPCConfigFile = function parseJPCConfigFile (source, user) {
        var jpc     = null,
            options = {};
        ...
        (jpc || {})[protolib.PROTO_IDENTIFIER].each(function (c, k) {
            if(typeof c === 'object' && (c.default !== undefined || c[protolib.ENV] !== undefined  || (c.debug !== undefined && protolib.DEBUG))) {
                options[k] = protolib.DEBUG && c.debug !== undefined ? c.debug : c[protolib.ENV] ? c[protolib.ENV] : c.default;
                if(typeof options[k] === 'string') options[k] = options[k][protolib.PROTO_IDENTIFIER].tildeToHome();
            }
            else {
                options[k] = c;
            }
        ...
        if(typeof user === 'object') user[protolib.PROTO_IDENTIFIER].each(function (u, k) { options[k] = u; });
        return options;
    };
};
